...
public class MyPageController extends AbstractController implements Initializable {
        ...
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                badges[i][j].addEventHandler(MouseEvent.MOUSE_ENTERED, e -> {
                    infolabel.setText("  Reward for " + this.activities[finalI]
                            + " for " + days + " days in a row!");
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        this.activities = new String[] {"eating vegetarian", "buying local produce",
                                        "taking the bike", "using public transport",
                                        "using solar panels", "lowering temperature"};
