        ...
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                badges[i][j].addEventHandler(MouseEvent.MOUSE_ENTERED, e -> {
                    infolabel.setText("  Reward for "+ this.activities[finalI] +" for "+ days +" days in a row!" );
                    infolabel.setVisible(true);
                });
                badges[i][j].addEventHandler(MouseEvent.MOUSE_EXITED, e -> infolabel.setVisible(false));
            }
        }
    }
